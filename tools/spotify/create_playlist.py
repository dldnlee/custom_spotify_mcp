...
def load_tokens() -> Dict[str, str]:
    ...
    try:
        config_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))),
                                   'spotify-config.json')  # Updated path
        with open(config_path, 'r') as f:
            config = json.load(f)
            return {
                'access_token': config['accessToken'],
...
def refresh_access_token(tokens: Dict[str, str], token_expiry: datetime) -> None:
    client_id = os.getenv('SPOTIFY_CLIENT_ID')
    client_secret = os.getenv('SPOTIFY_CLIENT_SECRET')
...
def make_spotify_request(method: str, endpoint: str, tokens: Dict[str, str], token_expiry: datetime, data: Dict[str, Any] = None) -> Dict[str, Any]:
    if datetime.now() >= token_expiry:
        token_expiry = refresh_access_token(tokens, token_expiry)
...
def create_playlist(name: str, description: str = "", public: bool = False) -> str:
    ...
    """
    try:
        # Load tokens only when the tool is called
        tokens = load_tokens()
        token_expiry = datetime.now() + timedelta(hours=1)

        user_profile = make_spotify_request('GET', '/me', tokens, token_expiry)
        user_id = user_profile['id']

        playlist = make_spotify_request('POST', f'/users/{user_id}/playlists', tokens, token_expiry, {
            'name': name,
            'description': description,
            'public': public
        })
